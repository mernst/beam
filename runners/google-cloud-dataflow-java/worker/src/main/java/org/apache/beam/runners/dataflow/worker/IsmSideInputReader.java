  ...
  public <ViewT> ViewT get(final PCollectionView<ViewT> view, final BoundedWindow window) {
    ...
    try {
      ...
      if (viewFn instanceof SingletonViewFn
        ...
        // When replacements are made in the pipeline, the ViewFn stored in the serialized DoFnInfo is not altered,
        // so even if there are some *ViewFn2 during lookup, by now it has been replaced and materialized "the old way"
        checkArgument(
