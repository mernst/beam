  ...
  private static class CombineInputs<
    ...
    public void outputAccumulators(FinishBundleContext context) {
      ...
      for (Map.Entry<WindowedStructuralKey<K>, AccumT> preCombineEntry : accumulators.entrySet()) {
        WindowedStructuralKey<K> key = preCombineEntry.getKey();
        Instant timestamp = timestamps.get(key);
        assert timestamp != null
            : "@AssumeAssertion(nullness): accumulators and timestamps have the same keySet";
        context.output(
            KV.of(key.getKey(), combineFn.compact(preCombineEntry.getValue())),
            timestamps.get(key),
            key.getWindow());
      }
